...
- 템플릿 이름: Kakao Emoticon (slug: kakao-emoticon, snake: kakao_emoticon)
...
```json
{
  ...
  "graphs": {
    ...
    "convert": "./casts/convert/graph.py:convert_graph"
  },
  "env": ".env"
}
```
...
- 각 Cast는 `casts/convert/modules/state.py`에서 세 가지 구분된 상태 스키마를 사용합니다:
...
| 스킬 | 목적 | 사용 시점 |
|------|------|----------|
| `@architecting-act` | 아키텍처 설계 | Kakao Emoticon 프로젝트 시작, 새 cast 계획, 기존 cast 재설계, 구조 불명확, CLAUDE.md 필요 시 |
...
`Node` (동기) 및 `AsyncNode` (비동기)의 실제 예제는 `casts/convert/modules/nodes.py`를 참조하세요.
