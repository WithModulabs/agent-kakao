# Convert 모듈 (Convert Module)

## 개요
이 모듈은 Kakao Emoticon 의 Convert 진행 및 통찰 추출을 담당하는 LangGraph Graph입니다.
...
result = convert_graph().invoke(initial_state)
```

## 의존성 추가
이 패키지에 종속성을 추가하려면 아래 명령어를 사용합니다:
```bash
uv add <패키지명> --package convert
```
